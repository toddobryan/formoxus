...
//! it validates first — and travels inside the [`FormSpec`](super::FormSpec)
...
use super::form::{Handler, UncheckedHandler};
...
impl ButtonSpec {
    ...
    /// same fallback a field label gets, and the same one the derive path runs
    /// a button name through.
...
/// Mirrors the five names `form2!` accepts. The mapping to HTML and to classes
/// below is the same one the derive path settled on.
